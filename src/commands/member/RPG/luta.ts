...
const command: ICommand = {
  ...
  handle: async (data) => {
    ...
    const mentionedJids = (data.baileysMessage.message as any).extendedTextMessage?.contextInfo?.mentionedJid || [];
    if (mentionedJids.length === 0) {
      throw new InvalidParameterError("Você precisa mencionar um usuário para lutar!");
    }

    const challengerId = data.user!;
    const opponentId = mentionedJids[0];

    // Garante que ambos os usuários existem na tabela user
    ...
    // Busca ou cria stats dos jogadores
    ...
    await prisma.playerStats.update({ where: { number: challengerId }, data: { hp: Math.max(0, hpA) } });
    await prisma.playerStats.update({ where: { number: opponentId }, data: { hp: Math.max(0, hpB) } });
    await prisma.playerStats.update({ where: { number: winnerId }, data: { xp: { increment: 10 } } });
    await prisma.playerStats.update({ where: { number: loserId }, data: { xp: { increment: 5 } } });
